...
public class MarchingCubes : MonoBehaviour
{
    ...
    {
        var vertexBuf = new NativeArray<Vector3>(size * size * size * 12, Allocator.Temp);
        var triangleBuf = new NativeArray<int>(size * size * size * 18, Allocator.Temp);
        int vertexCount = 0;
        int triangleCount = 0;
        ...
        for (int x = 0; x < size - 1; x++)
            for (int y = 0; y < size - 1; y++)
                for (int z = 0; z < size - 1; z++)
                {
                    ...
                    for (int i = 0; i < triangleTable[kind].Length; i++)
                        triangleBuf[triangleCount++] = vertexCount + i;

                    foreach (var i in triangleTable[kind])
                    {
                        Vector3 p0 = ToVec(x, y, z, edgeTable[i][0]);
                        Vector3 p1 = ToVec(x, y, z, edgeTable[i][1]);
                        float g0 = voxel[ToIdx(x, y, z, edgeTable[i][0], size)];
                        float g1 = voxel[ToIdx(x, y, z, edgeTable[i][1], size)];
